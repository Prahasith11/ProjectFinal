...
app.use(function (req, res, next) {
	...
	jwt.verify(token, app.get('secret'), function (err, decoded) {
		if (err) {
			...
			logger.debug(util.format('Decoded from JWT token: username - %s, orgname - %s', decoded.username, decoded.password));
			return next();
		}
	});
});
...
app.post('/login',async function (req,res){
	...
	console.log("hi")
	let response = await helper.loginUser(username,orgName);
	console.log("hi")
...
app.post('/users', async function (req, res) {
	var username = req.body.username;
	var orgName = req.body.userorg;
	var password = req.body.password  // edited
	...
	var token = jwt.sign({
		...
		username: username,
		password: password        // edited recently
	}, app.get('secret'));
...
app.post('/channels/:channelName/chaincodes/:chaincodeName', async function (req, res) {
	try {
		...
		let message = await invoke.invokeChaincode(peers, channelName, chaincodeName, fcn, args, req.username, 'Org1');
...
app.post('/query/channels/:channelName/chaincodes/:chaincodeName', async function (req, res) {
	...
	let message = await query.queryChaincode(peer, channelName, chaincodeName, args, fcn, req.username, 'Org1');
	res.send(message);
});
